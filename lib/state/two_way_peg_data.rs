...
use crate::{
    ...
    types::{
        ...
        AmountOverflowError, InPoint, M6id, OutPoint, OutPointKey, Output,
            ...
            if block_height == 0 {
                ...
                for (outpoint, output) in &utxos {
                    ...
                    state
                        ...
                        .map_err(DbError::from)?;
                    let utxo_hash = crate::types::hash_with_scratch_buffer(
                        &PointedOutputRef {
                            outpoint: *outpoint,
...
mod test {
    ...
    fn disconnect_failed_bundle_spends_reinstated_utxo() -> anyhow::Result<()> {
        ...
        let output = value_output(TransparentAddress::ALL_ZEROS, 1000);
        let key = OutPointKey::from(&outpoint);
    ...
    fn deposit_reorg_round_trips() -> anyhow::Result<()> {
        ...
        let merkle_root = empty_body.compute_merkle_root();
