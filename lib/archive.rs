...
use sneed::{
    DatabaseUnique, DbError, EnvError, RoDatabaseUnique, RoTxn, RwTxn,
    RwTxnError, UnitKey, db, env,
};
...
#[derive(Clone)]
pub struct Archive {
    /// Utreexo accumulators for blocks that have been successfully applied.
    accumulators: DatabaseUnique<
        SerdeBincode<Option<BlockHash>>,
        SerdeBincode<Accumulator>,
    >,
    ...
    >,
    /// Orchard note commitment merkle frontiers for blocks that have been
    /// successfully applied. Frontiers are only stored for blocks that change
    /// the frontier.
    orchard_frontiers: DatabaseUnique<
        SerdeBincode<Option<BlockHash>>,
        SerdeBincode<orchard::Frontier>,
    >,
...
impl Archive {
    ...
    pub fn new(env: &sneed::Env) -> Result<Self, Error> {
        ...
        if !accumulators.contains_key(&rwtxn, &None)? {
            accumulators.put(&mut rwtxn, &None, &Accumulator::default())?
        }
        ...
        let orchard_frontiers =
            DatabaseUnique::create(env, &mut rwtxn, "orchard_frontiers")?;
        if orchard_frontiers.try_get(&rwtxn, &None)?.is_none() {
            orchard_frontiers.put(
                &mut rwtxn,
                &None,
        ...
        if successors.try_get(&rwtxn, &None)?.is_none() {
    ...
    pub fn orchard_frontiers(
        &self,
    ) -> &RoDatabaseUnique<
        SerdeBincode<Option<BlockHash>>,
        SerdeBincode<orchard::Frontier>,
    > {
        &self.orchard_frontiers
    }
    ...
    pub fn get_accumulator(
        ...
        self.try_get_accumulator(rotxn, block_hash)?
            .ok_or_else(|| Error::NoAccumulator(block_hash.unwrap()))
    }
    ...
    pub fn try_get_body(
        ...
        let body = self.bodies.try_get(rotxn, &block_hash)?;
        Ok(body)
    }
    ...
    pub fn try_get_main_block_info(
        ...
        let block_info = self.main_block_infos.try_get(rotxn, main_hash)?;
        Ok(block_info)
    }
    ...
    pub fn try_get_main_header_info(
        ...
        let header_info = self.main_header_infos.try_get(rotxn, block_hash)?;
        Ok(header_info)
    }
    ...
    pub fn get_nth_ancestor(
        ...
        while n > 0 {
            ...
            if n == 1 {
                ...
                block_hash = self
                    .exponential_ancestors
                    .get(rotxn, &block_hash)?[exp_ancestor_index as usize];
    ...
    pub fn get_block_locator(
        ...
        let mut res = self.exponential_ancestors.get(rotxn, &block_hash)?;
    ...
    /// Store an accumulator for a connected block.
    pub fn put_accumulator(
        ...
        self.accumulators
            .put(rwtxn, &Some(block_hash), accumulator)?;
        Ok(())
    }
    ...
    pub fn put_orchard_frontier(
        &self,
        rwtxn: &mut RwTxn,
        block_hash: BlockHash,
        frontier: &orchard::Frontier,
    ) -> Result<(), Error> {
        let () =
            self.orchard_frontiers
                .put(rwtxn, &Some(block_hash), frontier)?;
        Ok(())
    }
